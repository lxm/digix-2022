...
txt_save_path = '/tudelft.net/staff-umbrella/SAMEN/DeepLearning/HUAWEI/2022_2_data/Train_ImageIndexwith_MultiClassLabels.txt'  # 生成的图片列表清单txt文件名
...
def read_directory(directory_name):
	...
	with open(os.path.join('/tudelft.net/staff-umbrella/SAMEN/DeepLearning/HUAWEI/2022_2_data/train_label','train_label.csv'), 'r', encoding='utf8') as fp:
		...
		for row in islice(reader, 0, None):
			print(len(row))
			imagename = row[0]
			for rownum in range(1, len(row)):
				print(rownum)
				class_label = row[rownum]
				fw.write(images_path + imagename + ' ')  # 打印成功信息
				fw.write(images_path + imagename + ' ') # 打印成功信息
				fw.write(images_path + imagename + ' ')  # 打印成功信息
				fw.write(images_path + imagename + ' ')  # 打印成功信息
				fw.write(images_path + imagename + ' ') 
				fw.write(str(class_label) + '\n')                 
