...
class ImageSet(data.Dataset):
    ...
    def get_txt_info(self, metadir, metafile):
        ...
        with open(os.path.join(metadir, metafile), 'r', encoding='utf8') as fp:
            ...
            for row in islice(reader, 0, None): # 1
                imagename = row[0]
...
def build_loader(
    ...
    trfs = [
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) #test no norm only find better with it training on mobilenetv3
    ]
